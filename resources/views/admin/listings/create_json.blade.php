...
<div class="form-card" style="margin: 0 auto; padding: 0; border: none; background: transparent; box-shadow: none; max-width: 100%;">
    <form action="{{ route('admin.listings.json.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="background: #ffffff; padding: 30px; border-radius: 16px; border: 1px solid var(--border); margin-bottom: 30px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
            ...
            <div class="form-group-horizontal" style="margin-top: 25px; display: flex; flex-direction: column; gap: 10px; margin-bottom: 25px;">
                ...
                <textarea name="json_data" id="json_data" rows="13" class="form-control @error('json_data') is-invalid @enderror" style="font-family: monospace; font-size: 0.9rem; padding: 15px; border-radius: 12px; border: 1px solid #e2e8f0; width: 100%; box-sizing: border-box; background: #f8fafc;" placeholder='{
  ...
  "referensi": "[https://www.facebook.com/Qautorentcarbatam1](https://www.facebook.com/Qautorentcarbatam1)"
                ...
                <small style="color: var(--text-muted); display: block; margin-top: 5px;">
                    Pastikan JSON menggunakan format yang valid dengan key wajib: <code>judul</code>, <code>alamat</code>, dan <code>keterangan_usaha</code>. Sertakan <code>nomor_wa</code> dan <code>nama</code> jika ingin membuat akun pengguna baru. Key <code>kecamatan</code> (opsional) untuk menyematkan listing ke kecamatan tertentu. Key <code>referensi</code> dapat digunakan untuk otomatis mengisi data tautan.
                </small>
                @error('json_data')
